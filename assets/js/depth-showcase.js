(() => {
  ...
  function render(){
    ...
    const orbit=-18+(p*150)+(pointerX*7);
    const polar=72-(Math.sin(p*Math.PI)*9)+(pointerY*4);
    const radius=110-(Math.sin(p*Math.PI)*10);
    stage.style.setProperty("--lotus-progress",p.toFixed(4));
    stage.style.setProperty("--lotus-scale",(0.78+Math.sin(p*Math.PI)*.1+p*.03).toFixed(4));
