(() => {
  ...
  // Hide browser broken-image UI gracefully if a future asset path is wrong.
  $$(".magnetic-gallery-v8 img").forEach(img=>{
    img.addEventListener("error",()=>{
      img.classList.add("asset-missing");
      const card=img.closest(".magnetic-card");
      if(card) card.classList.add("asset-missing-card");
    });
  });
